...
def pool_alice(X_unlabeled: np.ndarray, oracle: np.ndarray, basis_funcs: List[np.ufunc], n_tr: int, prng_seed:int, passive_learning:bool=False)->Tuple[np.ndarray,np.float_]: # type: ignore
    """
    ...
    for λ_val in λ_vals:
        ...
        if min_P_ALICE_λ > P_ALICE_λ:
            min_P_ALICE_λ = P_ALICE_λ
            best_idxs = tr_idxs
            best_L_λ = L_λ
            best_λ_val = λ_val
    ...
    # calculate the batch correlation (NOTE: this must be on the TRANSFORMED matrix. otherwise, there will be multiple copies of features that we are taking higher powers of. For example, if X_1, X_1^2, and X_1^3 are being used, then X_pretransformed contains three columns of X_1, X_1, X_1 and then X_transformed will transform those columns to X_1, X_1^2, X_1^3)
    best_X_λ = ϕ_mtx[best_idxs]
    avg_batch_cor = get_batch_correlation_v3(best_X_λ)

    return theta_W, avg_batch_cor
...
def simulate_pool_alice_n_times(n: int, seeds: List[int], X_pool: np.ndarray, y_pool: np.ndarray, basis_funcs:List[np.ufunc],passive_learning:bool=False, batch_size: int = 1)->Tuple[np.ndarray,np.ndarray]:

    sim_results: List[List[float]] = [None]*n # type: ignore
    cov_results: List[List[float]] = [None]*n # type: ignore
    ...
    for i in range(0,n):
        ...
        sim_results[i], cov_results[i] = simulate_pool_alice_once(X_train=X_train,y_train=y_train, X_test_transformed=X_test_transformed,y_test=y_test,basis_funcs=basis_funcs,prng_seed=prng_seed,passive_learning=passive_learning, batch_size=batch_size,start_time=start_time)
    ...
    return np.array(sim_results), np.array(cov_results)

def simulate_pool_alice_once(X_train: np.ndarray, y_train: np.ndarray, X_test_transformed: np.ndarray, y_test: np.ndarray, basis_funcs:List[np.ufunc],prng_seed:int,passive_learning:bool=False, batch_size: int = 1, start_time:float=0)->Tuple[List[float],List[float]]: # type: ignore
    ...
    avg_cor_vals: List[float] = [0.0]*len(num_samples_axis)
    ...
    for idx, n_tr in enumerate(num_samples_axis):
        ...
        theta_W, avg_batch_cor = pool_alice(X_unlabeled=X_train,oracle=y_train,basis_funcs=basis_funcs,n_tr=n_tr,prng_seed=prng_seed,passive_learning=passive_learning)
        ...
        avg_cor_vals[idx] = float(avg_batch_cor)

    return mse_vals, avg_cor_vals
...
def generate_or_load_result(result_file_name:str, sim_args:Dict={},overwrite_results:bool=False)->Tuple[np.ndarray,np.ndarray]:

    if len(sim_args) != 7:
        raise Exception('Please provide arguments for the simulation, the correct number of arguments is 7')
    else:
        n = sim_args['n']
        seeds = sim_args['seeds']
        X_pool = sim_args['X_pool']
        y_pool = sim_args['y_pool']
        basis_funcs = sim_args['basis_funcs']
        passive_learning = sim_args['passive_learning']
        batch_size = sim_args['batch_size']

    if result_file_name[-4:] != '.npy':
        result_name = result_file_name
        result_file_name += '.npy'
    else:
        result_name = result_file_name[:-4]

    result_file_name_cor = result_name + '_cor.npy'

    try:
        if overwrite_results:
            raise Exception('Force the except block to trigger')

        result_mse_vals: np.ndarray = np.load(f'../results/{result_file_name}')
        result_cor_vals: np.ndarray = np.load(f'../results/{result_file_name_cor}')
        print('RESULTS ALREADY OBTAINED')
    except:
        print('generating results...')
        result_mse_vals, result_cor_vals = simulate_pool_alice_n_times(
            n=n,
            seeds=seeds,
            X_pool=X_pool,
            y_pool=y_pool,
        ...
        np.save('../results/'+result_file_name,result_mse_vals)
        np.save('../results/'+result_file_name_cor,result_cor_vals)

    return result_mse_vals, result_cor_vals